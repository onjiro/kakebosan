defmodule KakebosanWeb.Accounting.InventoryControllerTest do
  ...
  setup %{conn: conn} do
    ...
    Repo.insert! %Inventory{id: 0, user_id: 1, item_id: 1,
  ...
  test "lists all entries on index", %{conn: conn} do
    ...
    assert json_response(conn, 200)["data"] == [
      %{"id" => 0, "date" => "2017-04-01T00:00:00.000000", "amount" => 100,
  ...
  test "shows chosen resource", %{conn: conn} do
    conn = get conn, inventory_path(conn, :show, 0)
    assert json_response(conn, 200)["data"] ==
      %{"id" => 0, "date" => "2017-04-01T00:00:00.000000", "amount" => 100,
        "item" => %{"id" => 1, "name" => "現金", "selectable" => true, "type_id" => 1, "description" => nil},
        "clearance_transaction" => nil }
  ...
  test "updates and renders chosen resource when data is valid", %{conn: conn} do
    conn = put conn, inventory_path(conn, :update, 0), inventory: @valid_attrs
    assert json_response(conn, 200)["data"]["id"]
    assert Repo.get_by(Inventory, @valid_attrs)
  end

  test "does not update chosen resource and renders errors when data is invalid", %{conn: conn} do
    conn = put conn, inventory_path(conn, :update, 0), inventory: @invalid_attrs
    assert json_response(conn, 422)["errors"] != %{}
  end

  test "deletes chosen resource", %{conn: conn} do
    conn = delete conn, inventory_path(conn, :delete, 0)
    assert response(conn, 204)
    refute Repo.get(Inventory, 0)
  end
end
